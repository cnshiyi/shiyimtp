...
# ================================
configure_systemctl() {
    local service_file="/etc/systemd/system/mtg_${port}.service"

    cat > "$service_file" <<EOF
[Unit]
Description=MTProxy Local Directory Service on port ${port}
...
EOF
    ...
    systemctl enable "mtg_${port}"
    systemctl restart "mtg_${port}"
}
...
————————————"
    ...
    case "$num" in
        1) install_mtg ;;
        2)
            systemctl stop "mtg_${port}"
            systemctl disable "mtg_${port}"
            rm -f "$MTG_BIN" "$MTG_CONF" "/etc/systemd/system/mtg_${port}.service"
            echo "卸载成功！"
        ;;
        3) systemctl start "mtg_${port}" && echo "启动成功！" && get_current_config ;;
        4) systemctl stop "mtg_${port}" && echo "停止成功！" ;;
        5) systemctl restart "mtg_${port}" && echo "重启成功！" && get_current_config ;;
        6) change_port ;;
        7) change_secret ;;
        8) update_mtg ;;
